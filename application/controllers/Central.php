...
class Central extends CI_Controller {	
    ...
    {
        $status = $this->m_central->get_all_status();
        $list = $this->m_central->get_datatables();
        $data = array();
        $no = $_POST['start'];
        foreach ($list as $wo) {
            $no++;
            ...
            //add html for action 
            if($wo->PASSWORD_VOICE == NULL)
            {
                $row[] = '<a class="btn btn-sm btn-danger" href="javascript:void(0)" title="AddPassword" onclick="add_password('."'".$wo->ID_TRANSAKSI."'".')"><i class="glyphicon glyphicon-pencil"></i> Tambah Password</a>
            ...
            {
                $row[] = '<a class="btn btn-sm btn-success" href="javascript:void(0)" title="EditPassword" onclick="edit_password('."'".$wo->ID_TRANSAKSI."'".')"><i class="glyphicon glyphicon-pencil"></i> Edit Password</a>
                  <a class="btn btn-sm btn-danger" href="javascript:void(0)" title="ChangeKendala" onclick="ChangeKendala('."'".$wo->ID_TRANSAKSI."'".')"><i class="glyphicon glyphicon-question-sign"></i> Tambah Kendala</a>
                  <a class="btn btn-sm btn-danger" href="javascript:void(0)" title="AddKeterangan" onclick="add_keterangan('."'".$wo->ID_TRANSAKSI."'".')"><i class="glyphicon glyphicon-pencil"></i> Tambah Keterangan</a>
                  <a class="btn btn-sm btn-primary" href="javascript:void(0)" title="Detail" onclick="detail('."'".$wo->ND."'".')"><i class="glyphicon glyphicon-ok"></i> Detail Pelanggan</a>';
        ...
        $output = array(
                        "draw" => $_POST['draw'],
                        "recordsTotal" => $this->m_central->count_all(),
                        "recordsFiltered" => $this->m_central->count_filtered(),
                        "data" => $data,
                );
 ...
 //SAMPE SINI WOIIIII
    ...
    {
        $data = $this->m_central->get_by_id_transaksi($id_transaksi);
    ...
    public function ajax_add_password()
    {
        ...
        $data = array(
                'PASSWORD_VOICE' => $this->input->post('password'),
        ...
        $log = array(
        		...
        		'action_log' => 'ADD PASSWORD',
        		'updated_by_log'=> $this->session->userdata('user')
        	);
        $this->m_central->update(array('ND'=> $this->input->post('ND')), $data);

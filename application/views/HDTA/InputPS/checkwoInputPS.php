...
$(document).ready(function() {
    ...
    table = $('#table').DataTable({ 
        ...
        "ajax": {
            "url": "<?php echo site_url('InputPS/ajax_list')?>",
            "type": "POST"
        },
...
function edit_sc(id)
{
    ...
    $.ajax({
        url : "<?php echo site_url('InputPS/ajax_edit/')?>/" + id,
        type: "GET",
        dataType: "JSON",
...
function detail(nd)
{
    ...
    //Ajax Load data from ajax
    $.ajax({
        url : "<?php echo site_url('InputPS/ajax_detail/')?>/" + nd,
        type: "GET",
        dataType: "JSON",
        success: function(data)
        {
...
function save()
{
    ...
    if(save_method == 'editsc') {
        url = "<?php echo site_url('InputPS/ajax_edit_sc')?>";
        ...
        $.ajax({
            ...
            success: function(data)
            {
                ...
                if(data.status) //if success close modal and reload ajax table
                {
                    ...
                    $('.alert-success').html('SC sukses diedit').fadeIn().delay(4000).fadeOut('slow');
                    reload_table();
                }
                else
                {
    ...
    }else if(save_method == 'editps') {
        url = "<?php echo site_url('InputPS/ajax_edit_ps')?>";
        // ajax adding data to database
        $.ajax({
            url : url,
            type: "POST",
            ...
            success: function(data)
            {
                ...
                if(data.status) //if success close modal and reload ajax table
                {
                    ...
                    $('.alert-success').html('PS sukses diedit').fadeIn().delay(4000).fadeOut('slow');
                    reload_table();
                }
                else
                {
    ...
    {
        url = "<?php echo site_url('InputPS/ajax_add_keterangan')?>";
        // ajax adding data to database
        $.ajax({
            url : url,
            type: "POST",
    ...
    {
        url = "<?php echo site_url('InputPS/ajax_change_kendala')?>";
        // ajax adding data to database
        $.ajax({
            url : url,
            type: "POST",
...
<!-- Bootstrap modal edit SC-->
<div class="modal fade" id="modal_form" role="dialog">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
                <h3 class="modal-title">Edit SC</h3>
            </div>
...
<!-- End Bootstrap modal edit SC-->
<!-- Bootstrap modal EDIT PS -->
<div class="modal fade" id="modal_edit" role="dialog">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
                <h3 class="modal-title">Edit PS</h3>
            </div>
            <div class="modal-body form">
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-md-9">
                                <select name="ps" class="form-control">
                                    <option value="">Pilih Status PS</option>
                                    <?php 
                                        foreach($select_status_ps as $object => $value) {
